...
class CACAuthenticationBackendDNParserTestCase(TestCase):
    ...
    def test_parses_dod_id_for_standard_dn(self):
    ...
    def test_parses_dod_id_for_no_middle_name(self):
        result = self.auth_backend.get_dodid_from_dn(
            "OU=USMC,CN=fname.lname.0123456789,OU=GOV"
        )
        self.assertEqual(result, "0123456789")

    def test_auth_fails_for_no_dod_id_found(self):
        self.assertRaises(
    ...
    def test_auth_fails_for_too_short_dod_id(self):
        self.assertRaises(
            AuthenticationFailed,
            self.auth_backend.get_dodid_from_dn,
    ...
    def test_auth_fails_for_empty_dn(self):
        self.assertRaises(AuthenticationFailed, self.auth_backend.get_dodid_from_dn, "")

    def test_auth_fails_for_empty_cn(self):
        self.assertRaises(
...
class CACAuthenticationBackendTestCase(TestCase):
    ...
    def test_returns_exisiting_user_with_dodid(self):
